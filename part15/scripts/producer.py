...
while True:
    index += 1
    type = f'q{random.randint(1,2)}'
    res = client.publish(
        TopicArn='arn:aws:sns:us-east-1:381606206112:topic-for-sns',
        Message=json.dumps(
            {"default": json.dumps({"type": 'q1', "mst": index})}),
        Subject='from scripts/producer.py',
