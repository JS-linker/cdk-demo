...
rootSteamName = 'LL-SC-S3BatchStack-LLSCStream03F1E1EB-vO7VcGk2UFBc'
...
while True:
    ...
    while True:
        payload = {
            "asin": str(uuid.uuid4()),
            "report_date": "2022-03-06",
            "marketplace": fake.city(),
            "user_id": fake.name(),
            "page_views": 210,
            "units_ordered": 11,
            "ordered_product_sales": "%s" % fake.random_int(min=1000, max=10000, step=1),
            "sales_currency": fake.city(),
            "order_item_session_percentage": "8.4",
            "sales_rank": 53782,
            "reviews_received": 0,
            "buy_box_precentage": "%s" % fake.random_int(min=1, max=99, step=1),
            "units_ordered_b2b": 0,
        ...
        # f.write('%s\n' % json.dumps(payload))
        # size = os.stat('big_data.json').st_size / 1024 / 1024
        # print(size)
        # if(size > 100):
        #     return
        records.append({
    ...
    response = kinesis.put_records(
        StreamName=rootSteamName, Records=records
    )
    pprint(response)
    if(loop >= loopCount):
        break
